...
//	connect(m_pwidget->optMarginCombo, SIGNAL(activated(int)), this, SLOT(slotOpticalMargin(int)));
	...
	connect(m_pwidget, SIGNAL(useParentWidowsOrphans()), this, SLOT(slotParentWidowsOrphans()));
	...
	connect(m_pwidget->hyphModeCombo, SIGNAL(activated(int)), this, SLOT(slotHyphenationMode(int)));
	connect(m_pwidget->maxHyphensSpin, SIGNAL(valueChanged(int)), this, SLOT(slotMaxHyphens(int)));

	connect(m_pwidget->maxTrackingSpinBox     , SIGNAL(valueChanged(double)), this, SLOT(slotMaxTracking()) );
	connect(m_pwidget->maxWordTrackingSpinBox     , SIGNAL(valueChanged(double)), this, SLOT(slotMaxWordTracking()) );
	...
	connect(m_pwidget->hyphModeCombo, SIGNAL(activated(int)), this, SLOT(slotHyphenationMode(int)));
	connect(m_pwidget->ClearOnApplyBox, SIGNAL(toggled(bool)), this, SLOT(slotClearOnApply(bool)));
	connect(m_pwidget, SIGNAL(useParentClearOnApply()), this, SLOT(slotParentClearOnApply()));
}
...
//	disconnect(m_pwidget->optMarginCombo, SIGNAL(activated(int)), this, SLOT(slotOpticalMargin(int)));
	...
	disconnect(m_pwidget, SIGNAL(useParentAdvancedSettings()), this, SLOT(slotParentAdvancedSettings()));
	...
	disconnect(m_pwidget->hyphModeCombo, SIGNAL(activated(int)), this, SLOT(slotHyphenationMode(int)));
	disconnect(m_pwidget->maxHyphensSpin, SIGNAL(valueChanged(int)), this, SLOT(slotMaxHyphens(int)));

	disconnect(m_pwidget->maxTrackingSpinBox     , SIGNAL(valueChanged(double)), this, SLOT(slotMaxTracking()) );
	disconnect(m_pwidget->maxWordTrackingSpinBox     , SIGNAL(valueChanged(double)), this, SLOT(slotMaxWordTracking()) );
	...
	disconnect(m_pwidget, SIGNAL(useParentWidowsOrphans()), this, SLOT(slotParentWidowsOrphans()));
	...
	disconnect(m_pwidget->hyphModeCombo, SIGNAL(activated(int)), this, SLOT(slotHyphenationMode(int)));
	disconnect(m_pwidget->ClearOnApplyBox, SIGNAL(toggled(bool)), this, SLOT(slotClearOnApply(bool)));
	disconnect(m_pwidget, SIGNAL(useParentClearOnApply()), this, SLOT(slotParentClearOnApply()));
}
...
void SMParagraphStyle::slotLineSpacingMode(int mode)
{
	...
	if (m_pwidget->lineSpacingMode->useParentValue())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->parentLineSpacingMode();
	else
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->setLineSpacingMode(lsm);
	//m_pwidget->showLineSpacing(m_selection);
...
void SMParagraphStyle::slotLineSpacing()
{
	if (m_pwidget->lineSpacing->useParentValue())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->parentLineSpacing();
	else 
	{
		double a, b, value;
		int c;
...
void SMParagraphStyle::slotOpticalMarginSelector()
{
	...
	//m_pwidget->showOpticalMargin(m_selection);
	if (!m_selectionIsDirty)
	{
		m_selectionIsDirty = true;
		emit selectionDirty();
...
{
	for (int i = 0; i < m_selection.count(); ++i)
		m_selection[i]->parentOpticalMargins();
	
	m_pwidget->setOpticalMargins(m_selection[0]->opticalMargins());
	if (!m_selectionIsDirty)
	{
		m_selectionIsDirty = true;
		emit selectionDirty();
...
void SMParagraphStyle::slotMinSpace()
{
	if (m_pwidget->minSpaceSpin->useParentValue())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->parentMinWordTracking();
	else 
	{
	...
	//m_pwidget->showMinSpace(m_selection);
	if (!m_selectionIsDirty)
	{
		m_selectionIsDirty = true;
		emit selectionDirty();
...
void SMParagraphStyle::slotMinGlyphExt()
{
	if (m_pwidget->minGlyphExtSpin->useParentValue())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->parentMinGlyphExtension();
	else 
	{
	...
	//m_pwidget->showMinGlyphExt(m_selection);
	if (!m_selectionIsDirty)
	{
		m_selectionIsDirty = true;
		emit selectionDirty();
...
void SMParagraphStyle::slotMaxGlyphExt()
{
	if (m_pwidget->maxGlyphExtSpin->useParentValue())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->parentMaxGlyphExtension();
	else 
	{
	...
	//m_pwidget->showMaxGlyphExt(m_selection);
	if (!m_selectionIsDirty)
	{
		m_selectionIsDirty = true;
		emit selectionDirty();
...
void SMParagraphStyle::slotParentAdvancedSettings()
{
	for (int i = 0; i < m_selection.count(); ++i)
	{
		m_selection[i]->parentMaxGlyphExtension();
...
void SMParagraphStyle::slotParentParaEffects()
{
	for (int i = 0; i < m_selection.count(); ++i)
	{
		...
		m_selection[i]->parentParEffectOffset();
		m_selection[i]->parentParEffectIndent();
		m_selection[i]->parentHasDropCap();
		m_selection[i]->parentDropCapLines();
		m_selection[i]->parentHasBullet();
		m_selection[i]->parentBulletStr();
		m_selection[i]->parentHasNum();
		m_selection[i]->parentNumName();
		m_selection[i]->parentNumFormat();
		m_selection[i]->parentNumPrefix();
		m_selection[i]->parentNumSuffix();
		m_selection[i]->parentNumLevel();
		m_selection[i]->parentNumRestart();
		m_selection[i]->parentNumOther();
		m_selection[i]->parentNumHigher();
	}
...
void SMParagraphStyle::handleKeepLinesStart()
{
	if (m_pwidget->keepLinesStart->useParentValue())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->parentKeepLinesStart();
	else 
	{
...
void SMParagraphStyle::handleKeepLinesEnd()
{
	if (m_pwidget->keepLinesEnd->useParentValue())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->parentKeepLinesEnd();
	else 
	{
...
void SMParagraphStyle::handleKeepTogether()
{
	if (m_pwidget->keepTogether->useParentValue())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->parentKeepTogether();
	else 
	{
...
void SMParagraphStyle::handleKeepWithNext()
{
	if (m_pwidget->keepWithNext->useParentValue())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->parentKeepWithNext();
	else 
	{
...
void SMParagraphStyle::slotParentWidowsOrphans()
{
	for (int i = 0; i < m_selection.count(); ++i)
	{
		m_selection[i]->parentKeepWithNext();
...
{
	if (m_pwidget->ClearOnApplyBox->useParentValue())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->parentClearOnApply();
	else
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->setClearOnApply(isOn);
	if (!m_selectionIsDirty)
	{
		m_selectionIsDirty = true;
		emit selectionDirty();
...
void SMParagraphStyle::slotLeftIndent()
{
	if (m_pwidget->tabList->useParentLeftIndent())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->parentLeftMargin();
	else 
	{
		double a, b, value;
		int c;
...
void SMParagraphStyle::slotRightIndent()
{
	if (m_pwidget->tabList->useParentRightIndent())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->parentRightMargin();
	else 
	{
		double a, b, value;
		int c;
...
void SMParagraphStyle::slotFirstLine()
{
	if (m_pwidget->tabList->useParentFirstLine())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->parentFirstIndent();
	else 
	{
		double a, b, value;
		int c;
...
void SMParagraphStyle::slotFontSize()
{
	if (m_pwidget->cpage->fontSize_->useParentValue())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->charStyle().parentFontSize();
	else
	{
		double a, b, value;
		int c;
...
void SMParagraphStyle::slotEffects(int e)
{
	...
	if (m_pwidget->cpage->effects_->useParentValue())
	{
		for (int i = 0; i < m_selection.count(); ++i)
		{
			m_selection[i]->charStyle().parentFeatures();
			m_selection[i]->charStyle().parentShadowXOffset();
			m_selection[i]->charStyle().parentShadowYOffset();
			m_selection[i]->charStyle().parentOutlineWidth();
			m_selection[i]->charStyle().parentUnderlineOffset();
			m_selection[i]->charStyle().parentUnderlineWidth();
			m_selection[i]->charStyle().parentStrikethruOffset();
			m_selection[i]->charStyle().parentStrikethruWidth();
		}
	}
	else
	{
...
void SMParagraphStyle::slotFillColor()
{
	if (m_pwidget->cpage->fillColor_->useParentValue())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->charStyle().parentFillColor();
	else {
...
void SMParagraphStyle::slotFillShade()
{
	if (m_pwidget->cpage->fillShade_->useParentValue())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->charStyle().parentFillShade();
	else {
...
void SMParagraphStyle::slotStrokeColor()
{
	if (m_pwidget->cpage->strokeColor_->useParentValue())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->charStyle().parentStrokeColor();
	else {
...
void SMParagraphStyle::slotStrokeShade()
{
	if (m_pwidget->cpage->strokeShade_->useParentValue())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->charStyle().parentStrokeShade();
	else 
	{
...
void SMParagraphStyle::slotScaleH()
{
	if (m_pwidget->cpage->fontHScale_->useParentValue())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->charStyle().parentScaleH();
	else
	{
		double a, b, value;
		int c;
...
void SMParagraphStyle::slotTracking()
{
	if (m_pwidget->cpage->tracking_->useParentValue())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->charStyle().parentTracking();
	else
	{
		double a, b, value;
		int c;
...
void SMParagraphStyle::slotBaselineOffset()
{
	if (m_pwidget->cpage->baselineOffset_->useParentValue())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->charStyle().parentBaselineOffset();
	else
	{
		double a, b, value;
		int c;
...
void SMParagraphStyle::slotFont(QString s)
{
	if (m_pwidget->cpage->fontFace_->useParentFont())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->charStyle().parentFont();
	else {
...
{
	if (m_pwidget->hyphModeCombo->useParentValue())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->resetHyphenationMode();
	else
		for (int i = 0; i < m_selection.count(); ++i)
		{
			m_selection[i]->setHyphenationMode(hm);
		}
	
	if (hm != ParagraphStyle::NoHyphenation)
		m_pwidget->maxHyphensSpin->setEnabled(true);
	else
		m_pwidget->maxHyphensSpin->setEnabled(false);
...
{
	if (m_pwidget->maxHyphensSpin->useParentValue())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->resetMaxHyphens();
	else
		for (int i = 0; i < m_selection.count(); ++i)
		{
			m_selection[i]->setMaxHyphens(mh);
		}
...
{
	if (m_pwidget->maxTrackingSpinBox->useParentValue())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->resetMaxTracking();
	else
	{
		double a, b, value;
		int c;
		
		m_pwidget->maxTrackingSpinBox->getValues(&a, &b, &c, &value);
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->setMaxTracking(value * 10);
	}
...
{
	if (m_pwidget->maxWordTrackingSpinBox->useParentValue())
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->resetMaxWordTracking();
	else
	{
		double a, b, value;
		int c;
		
		m_pwidget->maxWordTrackingSpinBox->getValues(&a, &b, &c, &value);
		for (int i = 0; i < m_selection.count(); ++i)
			m_selection[i]->setMaxWordTracking(value/100.0);
	}
