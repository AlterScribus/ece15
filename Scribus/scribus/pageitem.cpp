...
void PageItem::validateItemText()
{
	...
	for (int i=0; i < itemText.length(); )
	{
		...
		QChar ch = itemText.text(i);
		if (ch.isSpace() && ch != SpecialChars::TAB)
		{
			//spaces at start of paragraph
			if (vpref.removeSpacesParaStart && firstInPara)
			{
			...
			if (i > 0)
			{
				QChar prev = itemText.text(i-1);
				if (vpref.removeSpaceAfter.contains(prev))
				{
					itemText.removeChars(i,1);
					continue;
				}
			...
			if (i+1 < itemText.length())
			{
				QChar next = itemText.text(i+1);
				//multiple spaces
				if ((next.isSpace() && next != SpecialChars::TAB) && vpref.removeMultiSpaces)
				{
					itemText.removeChars(i,1);
					continue;
				}
				//spaces at end of paragraph
				if (next == SpecialChars::PARSEP && vpref.removeSpacesParaEnd)
				{
					itemText.removeChars(i,1);
					continue;
				}
				//remove spaces before some chars
				if (vpref.removeSpaceBefore.contains(next))
				{
					itemText.removeChars(i,1);
					continue;
				}
			...
			//last in paragraph
			else if (vpref.removeSpacesParaEnd)
			{
				itemText.removeChars(i,1);
				continue;
			}
			//converting to normal space
			if (vpref.convertSpacesToNormal && ch != ' ')
			{
				itemText.replaceChar(i, ' ');
				continue;
			}
			
		}
		//remove breaks
		else if (vpref.removeBreaks && (ch == SpecialChars::LINEBREAK))
		{
			if ((i > 0) && itemText.text(i-1).isSpace() && vpref.removeMultiSpaces)
				itemText.removeChars(i,1);
			else
				itemText.replaceChar(i, ' ');
			continue;
		...
		//remove hyphenation
		else if (vpref.removeHyphenation && ch == SpecialChars::SHYPHEN)
		{
			itemText.removeChars(i,1);
			continue;
		}
		...
		if ((i + 1 < itemText.length()) && vpref.removeCharBefore.contains(ch) && vpref.removeCharBefore.value(ch).contains(itemText.text(i+1)))
		{
			itemText.removeChars(i,1);
			continue;
		}
		//remove chars after some chars
		if (i>0 && vpref.removeCharAfter.contains(itemText.text(i-1)) && vpref.removeCharAfter.value(itemText.text(i-1)).contains(ch))
		{
			itemText.removeChars(i,1);
			continue;
		}
