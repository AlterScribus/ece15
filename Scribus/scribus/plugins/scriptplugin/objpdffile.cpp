...
// embed all used fonts
	...
	// get all used fonts
	QMap<QString,int> ReallyUsed = ScCore->primaryMainWindow()->m_Doc->UsedFonts;
...
// set to print all pages
	...
	self->pages = pages;
// print thumbnails ?
	self->thumbnails = ScCore->primaryMainWindow()->m_Doc->pdfOptions().Thumbnails;
// output crop marks ?
	self->cropmarks = ScCore->primaryMainWindow()->m_Doc->pdfOptions().cropMarks;
// cropmarks offset 
	self->cropmarksoffset = ScCore->primaryMainWindow()->m_Doc->pdfOptions().markOffset;
// output bleed marks ?
	self->bleedmarks = ScCore->primaryMainWindow()->m_Doc->pdfOptions().bleedMarks;
// output registration marks ?
	self->registrationmarks = ScCore->primaryMainWindow()->m_Doc->pdfOptions().registrationMarks;
// output color bars ?
	self->colormarks = ScCore->primaryMainWindow()->m_Doc->pdfOptions().colorMarks;
// output page information ?
	self->pageinformation = ScCore->primaryMainWindow()->m_Doc->pdfOptions().docInfoMarks;
// embed pdf and eps ?
	self->embedpdf = ScCore->primaryMainWindow()->m_Doc->pdfOptions().embedPDF;
// set automatic compression
	self->compress = ScCore->primaryMainWindow()->m_Doc->pdfOptions().Compress;
	self->compressmtd = ScCore->primaryMainWindow()->m_Doc->pdfOptions().CompressMethod;
// use maximum image quality
	self->quality = ScCore->primaryMainWindow()->m_Doc->pdfOptions().Quality;
// default resolution
...
// do not downsample images
	int down = ScCore->primaryMainWindow()->m_Doc->pdfOptions().RecalcPic ? ScCore->primaryMainWindow()->m_Doc->pdfOptions().PicRes : 0;
	...
	// no bookmarks
	self->bookmarks = ScCore->primaryMainWindow()->m_Doc->pdfOptions().Bookmarks;
	// left margin binding
	self->binding = ScCore->primaryMainWindow()->m_Doc->pdfOptions().Binding;
	// do not enable presentation effects
	self->presentation = ScCore->primaryMainWindow()->m_Doc->pdfOptions().PresentMode;
	...
	int num2 = ScCore->primaryMainWindow()->m_Doc->pdfOptions().PresentVals.count();
	for (i = 0; i<num2; ++i) {
		PyObject *tmp;
		PDFPresentationData t = ScCore->primaryMainWindow()->m_Doc->pdfOptions().PresentVals[i];
...
// do not save linked text frames as PDF article
	self->article = ScCore->primaryMainWindow()->m_Doc->pdfOptions().Articles;
// do not encrypt file
	self->encrypt = ScCore->primaryMainWindow()->m_Doc->pdfOptions().Encrypt;
// do not Use Custom Rendering Settings
	self->uselpi = ScCore->primaryMainWindow()->m_Doc->pdfOptions().UseLPI;
	self->usespot = ScCore->primaryMainWindow()->m_Doc->pdfOptions().UseSpotColors;
	self->domulti = ScCore->primaryMainWindow()->m_Doc->pdfOptions().doMultiFile;
// get default values for lpival
	int n = ScCore->primaryMainWindow()->m_Doc->pdfOptions().LPISettings.size();
	PyObject *lpival=PyList_New(n);
	if (!lpival){
		PyErr_SetString(PyExc_SystemError, "Can not initialize 'lpival' attribute");
		return -1;
	}
	QMap<QString,LPIData>::Iterator it = ScCore->primaryMainWindow()->m_Doc->pdfOptions().LPISettings.begin();
	while (it != ScCore->primaryMainWindow()->m_Doc->pdfOptions().LPISettings.end()) {
		PyObject *tmp;
...
// allow printing document
	self->aprint = ScCore->primaryMainWindow()->m_Doc->pdfOptions().Permissions & 4;
// allow changing document
	self->achange = ScCore->primaryMainWindow()->m_Doc->pdfOptions().Permissions & 8;
// allow copying document
	self->acopy = ScCore->primaryMainWindow()->m_Doc->pdfOptions().Permissions & 16;
// allow adding annotation and fields
	self->aanot = ScCore->primaryMainWindow()->m_Doc->pdfOptions().Permissions & 32;
// use 1.4 pdf version *aka. Acrobat 5)
	self->version = ScCore->primaryMainWindow()->m_Doc->pdfOptions().Version;
// output destination is screen
	self->outdst = ScCore->primaryMainWindow()->m_Doc->pdfOptions().UseRGB ? 0 : 1;

	self->profiles = ScCore->primaryMainWindow()->m_Doc->pdfOptions().UseProfiles; // bool
	self->profilei = ScCore->primaryMainWindow()->m_Doc->pdfOptions().UseProfiles2; // bool
	self->noembicc = ScCore->primaryMainWindow()->m_Doc->pdfOptions().EmbeddedI; // bool
	self->intents = ScCore->primaryMainWindow()->m_Doc->pdfOptions().Intent; // int - 0 - 3
	self->intenti = ScCore->primaryMainWindow()->m_Doc->pdfOptions().Intent2; // int - 0 - 3
	QString tp = ScCore->primaryMainWindow()->m_Doc->pdfOptions().SolidProf;
	...
	QString tp2 = ScCore->primaryMainWindow()->m_Doc->pdfOptions().ImageProf;
	...
	QString tp3 = ScCore->primaryMainWindow()->m_Doc->pdfOptions().PrintProf;
	...
	QString tinfo = ScCore->primaryMainWindow()->m_Doc->pdfOptions().Info;
	PyObject *info = NULL;
	...
	self->bleedt = ScCore->primaryMainWindow()->m_Doc->pdfOptions().bleeds.Top*ScCore->primaryMainWindow()->m_Doc->unitRatio(); // double -
	self->bleedl = ScCore->primaryMainWindow()->m_Doc->pdfOptions().bleeds.Left*ScCore->primaryMainWindow()->m_Doc->unitRatio(); // double -
	self->bleedr = ScCore->primaryMainWindow()->m_Doc->pdfOptions().bleeds.Right*ScCore->primaryMainWindow()->m_Doc->unitRatio(); // double -
	self->bleedb = ScCore->primaryMainWindow()->m_Doc->pdfOptions().bleeds.Bottom*ScCore->primaryMainWindow()->m_Doc->unitRatio(); // double -
...
//void ScribusMainWindow::SaveAsPDF()
	...
	if (ScCore->primaryMainWindow()->bookmarkPalette->BView->topLevelItemCount() == 0)
		ScCore->primaryMainWindow()->m_Doc->pdfOptions().Bookmarks = false;

// apply fonts attribute
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().EmbedList.clear();
	int n = PyList_Size(self->fonts);
	for ( int i=0; i<n; ++i){
		QString tmpFon;
		tmpFon = QString(PyString_AsString(PyList_GetItem(self->fonts, i)));
		ScCore->primaryMainWindow()->m_Doc->pdfOptions().EmbedList.append(tmpFon);
	}
// apply file attribute
	QString fn;
	fn = QString(PyString_AsString(self->file));
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().fileName = fn;
...
// apply thumbnails attribute
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().Thumbnails = self->thumbnails;
// apply cropmarks attribute
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().cropMarks = self->cropmarks;
// apply cropmarks offset attribute
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().markOffset = self->cropmarksoffset;
// apply bleedmarks attribute
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().bleedMarks = self->bleedmarks;
// apply registrationmarks attribute
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().registrationMarks = self->registrationmarks;
// apply colormarks attribute
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().colorMarks = self->colormarks;
// apply pageinformation attribute
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().docInfoMarks = self->pageinformation;
// apply embedpdf attribute
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().embedPDF = self->embedpdf;
// apply compress attribute
	self->compressmtd = minmaxi(self->compressmtd, 0, 3);
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().Compress = self->compress;
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().CompressMethod = (PDFOptions::PDFCompression) self->compressmtd;
// apply quality attribute
	self->quality = minmaxi(self->quality, 0, 4);
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().Quality = self->quality;
// apply resolusion attribute
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().Resolution = PyInt_AsLong(self->resolution);
// apply downsample attribute
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().RecalcPic = PyInt_AsLong(self->downsample);
	if (ScCore->primaryMainWindow()->m_Doc->pdfOptions().RecalcPic)
		ScCore->primaryMainWindow()->m_Doc->pdfOptions().PicRes = PyInt_AsLong(self->downsample);
	else
		ScCore->primaryMainWindow()->m_Doc->pdfOptions().PicRes = ScCore->primaryMainWindow()->m_Doc->pdfOptions().Resolution;
// apply bookmarks attribute
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().Bookmarks = self->bookmarks;
// apply binding attribute
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().Binding = self->binding;
// apply presentation attribute
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().PresentMode = self->presentation;
...
//		 PresentVals.append(t);
	...
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().PresentVals = PresentVals;
// apply lpival
...
//		 ScCore->primaryMainWindow()->doc->pdfOptions().LPISettings[QString(s)]=lpi;
		...
		ScCore->primaryMainWindow()->m_Doc->pdfOptions().LPISettings[st]=lpi;
	}

	ScCore->primaryMainWindow()->m_Doc->pdfOptions().Articles = self->article;
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().Encrypt = self->encrypt;
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().UseLPI = self->uselpi;
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().UseSpotColors = self->usespot;
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().doMultiFile = self->domulti;
	self->version = minmaxi(self->version, 12, 14);
	// FIXME: Sanity check version
	ScCore->primaryMainWindow()->m_Doc->pdfOptions().Version = (PDFOptions::PDFVersion)self->version;
	if (self->encrypt)
	{
		int Perm = -64;
		if (ScCore->primaryMainWindow()->m_Doc->pdfOptions().Version == PDFOptions::PDFVersion_14)
			Perm &= ~0x00240000;
		if (self->aprint)
			Perm += 4;
		if (self->achange)
		...
		if (self->aanot)
			Perm += 32;
		ScCore->primaryMainWindow()->m_Doc->pdfOptions().Permissions = Perm;
		ScCore->primaryMainWindow()->m_Doc->pdfOptions().PassOwner = QString(PyString_AsString(self->owner));
		ScCore->primaryMainWindow()->m_Doc->pdfOptions().PassUser = QString(PyString_AsString(self->user));
	}
	if (self->outdst == 0)
	{
		ScCore->primaryMainWindow()->m_Doc->pdfOptions().UseRGB = true;
		ScCore->primaryMainWindow()->m_Doc->pdfOptions().UseProfiles = false;
		ScCore->primaryMainWindow()->m_Doc->pdfOptions().UseProfiles2 = false;
	}
	else
	{
		ScCore->primaryMainWindow()->m_Doc->pdfOptions().UseRGB = false;
		if (ScCore->primaryMainWindow()->m_Doc->HasCMS)
		{
			ScCore->primaryMainWindow()->m_Doc->pdfOptions().UseProfiles = self->profiles;
			ScCore->primaryMainWindow()->m_Doc->pdfOptions().UseProfiles2 = self->profilei;
			self->intents = minmaxi(self->intents, 0, 3);
			ScCore->primaryMainWindow()->m_Doc->pdfOptions().Intent = self->intents;
			self->intenti = minmaxi(self->intenti, 0, 3);
			ScCore->primaryMainWindow()->m_Doc->pdfOptions().Intent2 = self->intenti;
			ScCore->primaryMainWindow()->m_Doc->pdfOptions().EmbeddedI = self->noembicc;
			ScCore->primaryMainWindow()->m_Doc->pdfOptions().SolidProf = PyString_AsString(self->solidpr);
			ScCore->primaryMainWindow()->m_Doc->pdfOptions().ImageProf = PyString_AsString(self->imagepr);
			ScCore->primaryMainWindow()->m_Doc->pdfOptions().PrintProf = PyString_AsString(self->printprofc);
			if (ScCore->primaryMainWindow()->m_Doc->pdfOptions().Version == PDFOptions::PDFVersion_X3)
			{
				ScColorProfile profile;
				profile = ScCore->defaultEngine.openProfileFromFile(ScCore->PrinterProfiles[ScCore->primaryMainWindow()->m_Doc->pdfOptions().PrintProf]);
				...
				if (profile.colorSpace() == ColorSpace_Cmy)
					Components = 3;
				ScCore->primaryMainWindow()->m_Doc->pdfOptions().Info = PyString_AsString(self->info);
				...
				ScCore->primaryMainWindow()->m_Doc->pdfOptions().bleeds.Top = self->bleedt/ScCore->primaryMainWindow()->view->Doc->unitRatio();
				...
				ScCore->primaryMainWindow()->m_Doc->pdfOptions().bleeds.Left = self->bleedl/ScCore->primaryMainWindow()->view->Doc->unitRatio();
				...
				ScCore->primaryMainWindow()->m_Doc->pdfOptions().bleeds.Right = self->bleedr/ScCore->primaryMainWindow()->view->Doc->unitRatio();
				...
				ScCore->primaryMainWindow()->m_Doc->pdfOptions().bleeds.Bottom = self->bleedb/ScCore->primaryMainWindow()->view->Doc->unitRatio();
				ScCore->primaryMainWindow()->m_Doc->pdfOptions().Encrypt = false;
				ScCore->primaryMainWindow()->m_Doc->pdfOptions().PresentMode = false;
			}
		}
		else
		{
	...
	{
		QPixmap pm(10,10);
		if (ScCore->primaryMainWindow()->m_Doc->pdfOptions().Thumbnails)
			pm = QPixmap::fromImage(ScCore->primaryMainWindow()->view->PageToPixmap(pageNs[ap]-1, 100));
		thumbs.insert(pageNs[ap], pm);
	}
	ReOrderText(ScCore->primaryMainWindow()->m_Doc, ScCore->primaryMainWindow()->view);
