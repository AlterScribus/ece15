...
bool XPSExportPlugin::run(ScribusDoc* doc, QString filename)
{
	...
	if (doc!=0)
	{
		...
		QScopedPointer<CustomFDialog> openDia( new CustomFDialog((QWidget*)doc->scMW(), wdir, QObject::tr("Save as"), QObject::tr("Microsoft XPS (*.xps *.XPS);;All Files (*)"), fdHidePreviewCheckBox) );
		...
		if (f.exists())
		{
			int exit = QMessageBox::warning((QWidget*)doc->scMW(), CommonStrings::trWarning,
